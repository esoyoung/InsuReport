...
const COVERAGE_GROUPS = [
  {
    title: '사망 · 장해',
    items: ['상해사망', '질병사망', '장기요양간병비', '간병인/간호간병질병일당', '상해80%미만후유장해', '질병80%미만후유장해'],
  },
  {
    title: '암 · 중증 질환',
    items: ['일반암', '유사암', '고액암', '고액(표적)항암치료비', '암진단', '일반암진단', '소액암', '기타피부암', '갑상선암', '경계성종양', '제자리암'],
  },
  {
    title: '뇌 · 심장 중심 진단',
    items: ['뇌혈관질환', '뇌졸중', '뇌출혈', '허혈성심장질환', '급성심근경색증', '심근경색증', '뇌출혈진단'],
  },
  {
    title: '치매 · 간병',
    items: ['경증치매진단', '중증치매', '치매진단', '간병인/간호간병상해일당', '장기요양간병비'],
  },
  {
    title: '입원 · 수술 · 실손',
    items: [
...
// 담보명 정규화 함수 (AI 추출 데이터 → 표준 담보명)
const normalizeCoverageName = (name) => {
  if (!name) return '';
            ...
            {COVERAGE_GROUPS.map((group) => (
              <React.Fragment key={group.title}>
                ...
                {group.items.map((name) => {
                  ...
                  const hasShortfall = row.부족금액값 > 0;
                  ...
                  return (
                    <tr key={`${group.title}-${name}`} className="align-top" style={{ fontSize: '0.7rem', lineHeight: '1.2' }}>
                      ...
                      <td className={classNames('px-1 py-0.5', hasShortfall ? 'text-red-600 font-bold' : 'text-gray-700')}>
                        {hasShortfall ? renderCellContent(shortfallDisplay, { align: 'right' }) : renderCellContent('', { align: 'right', fallback: '' })}
                      </td>
