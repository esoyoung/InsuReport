...
- ⚠️ **월보험료 합계 계산 (매우 중요)**:
  - **계산 방식**: 각 계약의 납입 만료 여부를 판단하여 합산
  - **계산 로직**:
    1. 각 계약마다 계약일(YYYY-MM-DD)과 납입기간(예: "20년", "30년")을 확인
    2. 납입 종료일 = 계약일 + 납입기간 (예: 2020-01-01 + 20년 = 2040-01-01)
    3. 오늘 날짜(2025-11-27)와 비교:
       - 납입 종료일 < 오늘 → 납입 완료 → 해당 월보험료 제외 (0원으로 계산)
       - 납입 종료일 ≥ 오늘 → 진행중 → 해당 월보험료 포함
    4. 진행중인 계약들의 월보험료만 합산하여 "총보험료"와 "활성월보험료"에 사용
  - **예시**:
    - 계약1: 계약일 2010-01-01, 납입기간 10년 → 납입 종료 2020-01-01 → 오늘(2025-11-27) > 종료일 → **제외** (0원)
    - 계약2: 계약일 2020-01-01, 납입기간 20년 → 납입 종료 2040-01-01 → 오늘(2025-11-27) < 종료일 → **포함** (월보험료)
    - 계약3: 계약일 2021-06-15, 납입기간 30년 → 납입 종료 2051-06-15 → 오늘(2025-11-27) < 종료일 → **포함** (월보험료)
    - 총보험료 = 계약2 월보험료 + 계약3 월보험료
  - **주의**: 납입기간이 "전기", "종신" 등인 경우 → 진행중으로 간주하여 포함
...
**총보험료/활성월보험료 계산 예시:**
- 계약 리스트에서 각 계약의 납입 완료 여부를 판단 (계약일 + 납입기간)
- 납입 종료일이 오늘(2025-11-27)보다 이전이면 → 해당 계약 월보험료 **제외**
- 납입 종료일이 오늘 이후이면 → 해당 계약 월보험료 **포함**
- 예시:
  - 계약A(2010년, 10년납) → 2020년 완료 → **제외** (0원)
  - 계약B(2020년, 20년납) → 2040년 만료 → **포함** (120,000원)
  - 계약C(2022년, 전기납) → 진행중 → **포함** (33,500원)
  - 총보험료 = 120,000 + 33,500 = 153,500
...
- **⚠️ 월보험료 합계 계산 필수**: 
  - 각 계약의 계약일 + 납입기간을 통해 납입 종료일을 계산
  - 납입 종료일이 오늘(2025-11-27)보다 이전이면 해당 월보험료 제외
  - 납입 종료일이 오늘 이후이면 해당 월보험료 포함
  - 진행중인 계약들의 월보험료만 합산하여 "총보험료", "활성월보험료"에 사용
