...
4. **실효해지계약** ("님의 실효/해지 계약 현황" 표)
   ...
   **⚠️ 중요 - 실효/해지 계약 추출**: 
   - **표 이름**: "님의 실효/해지 계약 현황" 또는 "실효·해지 계약"
   - **표 특징**: "해지일", "해지사유" 컬럼 포함
   - **위치**: 보통 보유 계약 리스트 표 하단에 위치
   - **필수**: 실효/해지 계약이 없더라도 빈 배열([]) 반환
   - **확인**: PDF를 세심하게 확인하여 해당 표 찾기
...
5. **진단현황** ("님의 상품별 진단현황" 표)
   ...
   **⚠️ 중요 - 진단현황 순서 보장**: 
   - **기본 순서**: 아래 31개 항목 순서를 기본 템플릿으로 사용
   - **AI 데이터 매칭**: PDF에서 추출한 데이터를 기본 순서에 매칭
   - **미가입 항목**: PDF에 없더라도 기본 템플릿에 포함, 가입금액=0
   - **부족금액 표시**: 부족 시 값 표시, 충분/미가입 시 0 또는 null
