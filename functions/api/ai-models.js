...
4. **실효해지계약** ("님의 실효/해지 계약 현황" 표)
   ...
   - **보험사**: 회사명
   - **상품명**: 상품명
   - **가입일**: 계약 시작일 (있으면 추출, 없으면 생략)
   - **해지사유**: 해지/실효 사유 (있으면 추출, 없으면 생략)
   - **상태**: "해지" 또는 "실효"
   
   **⚠️ 중요**: 
   - 해지일은 **추출하지 않음** (PDF에 표시되지 않는 경우가 많음)
   - 가입일과 해지사유도 **선택적 필드** (없으면 생략)
...
5. **진단현황** ("님의 상품별 진단현황" 표)
   ...
   **원칙: 아래 템플릿 순서를 반드시 따를 것**
   - **1순위**: 아래 35개 템플릿 순서에 따라 **반드시 순서대로** 담보를 배치
   - **2순위**: 각 담보의 권장금액과 가입금액을 PDF에서 **정확히** 매칭
   - 담보를 **절대 재정렬하지 말 것** (알파벳, 금액, 중요도 정렬 금지)
   - **템플릿에 있는 35개 담보는 모두 포함**할 것 (PDF에 없어도 포함)
     
   **미가입 항목 처리**:
   - PDF에 담보명이 없는 경우: 가입금액=0, 권장금액=0, 부족금액=0, 상태="미가입"
   - PDF에 담보명은 있으나 가입금액이 없는 경우: 가입금액=0, 부족금액=권장금액, 상태="미가입"
   
   **데이터 추출 필수 규칙**:
   - PDF 표에서 권장금액과 가입금액을 **반드시 함께 추출**할 것
   - 권장금액만 있고 가입금액이 비어있으면 가입금액=0으로 설정
   - 가입금액만 있고 권장금액이 비어있으면 권장금액=가입금액으로 설정
  ...
  "실효해지계약": [
    {
      "보험사": "string",
      "상품명": "string",
      "가입일": "YYYY-MM-DD" (선택적, 없으면 생략 가능),
      "해지사유": "string" (선택적, 없으면 생략 가능),
      "상태": "해지" | "실효"
    }
  ],
  "진단현황": [
